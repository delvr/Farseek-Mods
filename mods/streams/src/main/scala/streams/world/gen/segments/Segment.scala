...
abstract class Segment(using val randomState: RandomState):
  ...
  final lazy val xzBounds: BlockXZBox = chunkBounds.into[BlockXZBox]
  ...
  lazy val outletIsFall: Boolean = false
  lazy val inletHasFall: Boolean = false
  ...
  private def pathBetween(start: StreamsColumn, end: StreamsColumn, steps: BezierCurve[EuclidXZ]): Path =
    NonEmptySeq.interpolate(pathSteps): t =>
      ...
      StreamsColumn(this, steps(t).into[BlockXZ], maxFloorLevel, minClearLevel, airFlow)
