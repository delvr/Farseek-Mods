...
sealed abstract class TributaryNode(basin: TributaryBasin, chunkPos: ChunkXZ)(using RandomState)
...
final case class TributaryOutletNode(basin: TributaryBasin, chunkPos: ChunkXZ)(using RandomState)
...
final case class TributaryUpstreamNode(basin: TributaryBasin, chunkPos: ChunkXZ, maxSurfaceLevelFromSelf: BlockY,
    downstreamSide: XZSide)(using RandomState) extends TributaryNode(basin, chunkPos):

  override lazy val maxDepth: PositiveInt = generator.maxTributaryDepth

  override lazy val surfaceLevel: BlockY =
    val fallHeight = (downstreamNode.surfaceLevel delta maxSurfaceLevelFromUpstream) / 2
    downstreamNode.surfaceLevel :+ (if fallHeight >= MinFallHeight then fallHeight else 0)
  ...
  override lazy val outletIsFall: Boolean = downstreamNode.surfaceLevel < this.surfaceLevel
  override lazy val inletHasFall: Boolean = upstreamNodes.exists(_.outletIsFall)
