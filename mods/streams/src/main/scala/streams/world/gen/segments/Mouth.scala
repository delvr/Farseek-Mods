...
final case class Mouth(generator: StreamsGenerator, basinPos: BasinXZ,
    leaksLeftToRight: NonEmptySeq[NonEmptySeq[BlockXZ]])(using RandomState) extends BasinSegment:
  ...
  override protected def transform(path: Path): Path =
    ...
    val straightPath = path.map(step => step.copy(maxFloorLevel = inletFloorLevel))
    straightPath.firstIndexWhere(step =>  generator.likelyInSea(step.pos.at(
      if generator.seaFluid.hasLava then generator.minSurfaceLevel else inletFloorLevel))) match
