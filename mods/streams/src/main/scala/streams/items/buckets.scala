      ...
      stack.enchantmentLevel(FlowEnchantment).foreach: flowSpeed =>
        ...
        if hitResult.isEmpty then
          ...
          if !player.mayInteractAt(hitPos) ||
             !player.mayUseItemAt(posInFrontOfHit.into[BlockPos], hitSide.into[Direction], stack) then
            event.fail()
          else
            ...
            val isOrCanHaveFluid = hitState.block match
              case container: LiquidBlockContainer => container.canPlaceLiquid(
                player, level, hitPos.into[BlockPos], hitState, bucket.content)
              case _ => hitPos.isWet
            ...
            if currentFluid.isEmpty then
              if !bucket.emptyContents(player, level, pourPos.into[BlockPos], hitResult, stack) then
                event.fail()
            else if currentFluid.fluidIsStillOrFlowing(bucket.content) then
              bucket.playEmptySound(player, level, pourPos.into[BlockPos])
            else
              event.pass()
