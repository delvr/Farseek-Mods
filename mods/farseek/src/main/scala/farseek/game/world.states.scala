...
given StateAtBlockXYZ: (BlockGetter) => BlockXYZ Into BlockState = _.into[BlockPos].into[BlockState]
...
extension(state: into BlockState)
  ...
  def isSolidBlock: Boolean = blockState.isSolid: @nowarn

  def nonAirBlock  : Boolean = !isAirBlock
  def nonEmptyBlock: Boolean = !isEmptyBlock
  def isNonSolid   : Boolean = !isSolidBlock
