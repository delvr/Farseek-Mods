...
trait StateProperty[S <: PropHolder[S], V] extends (S IntoOption V):
  ...
  final def registerUsing(builder: StateDefinition.Builder[?, S]): Unit = builder.add(wrapped): Unit
...
final case class BoolProperty[S <: PropHolder[S]](
  ...
  override protected def convert(value:  Boolean) = value.into[JBoolean]
  override protected def convert(value: JBoolean) = value.booleanValue

final case class IntProperty[S <: PropHolder[S], V <: NonNegativeInt: Enumerable](
  ...
  override protected def convert(value: V): JInteger = value.into[JInteger]
  override protected def convert(value: JInteger): V = refined(value.intValue)
