...
extension(writer: LevelWriter | Chunk | WrappedChunk)
  ...
  def set   (p: into BlockPos, state: into BlockState, flags: Int): Boolean = writer match
    ...
    case chunk: WrappedChunk => chunk.setBlockState(p, state, flags) != null
    case chunk: Chunk        => chunk.setBlockState(p, state, flags) != null

extension(p: into BlockPos)(using writer: LevelWriter | Chunk | WrappedChunk)
  def setBlock(state: into BlockState, flags: Flags): Unit = writer.set(p, state, flags.toInt): Unit
