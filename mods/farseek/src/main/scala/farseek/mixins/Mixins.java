...
abstract class ChunkGeneratorMixins {
    ...
    private void buildSurface(WorldGenRegion region, StructureManager manager,
                              RandomState random, ChunkAccess chunkAccess, Operation<Void> original) {
        NoiseBasedChunkGenerator generator = cast(this);
        ProtoChunk chunk = (ProtoChunk)chunkAccess;
        if(BuildSurface.shouldProceed(generator, region, manager, random, chunk))
            original.call(region, manager, random, chunk);
        BuildSurface.postBuild(generator, region, manager, random, chunk);
    }
...
abstract class StructureMixins {
    ...
    private StructureStart generate(Holder<Structure> structure,
            ...
            ChunkGenerator chunks, BiomeSource biomes, RandomState random,
        ...
        return GenerateStructure.generate(cast(this),
            registries, chunks, biomes, random, templates, seed, pos, references, level, biomeFilter);
    }
    private static <T> T cast(Object x) { return (T)x; }
}
