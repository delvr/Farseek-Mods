...
extension(p: into BlockXYZ)(using BlockGetter, BlockYBox)
...
extension(entity: Entity) def canUseSlopes: Boolean =
  entity.isInstanceOf[LivingEntity] && !entity.isInstanceOf[WaterAnimal] && !entity.isInstanceOf[FlyingMob] &&
  !entity.isNoGravity && !entity.isSteppingCarefully && entity.moveDist > 0 && entity.maxUpStep >= StepHeight
