...
final case class TributaryBasin(generator: StreamsGenerator, basinPos: BasinXZ,
    downstreamSide: XZSide)(using NoiseState)
  ...
  lazy val (outletNode: TributaryOutletNode, pathNodes: Map[ChunkXZ, TributaryNode]) =
    ...
    val chunkMaxSurfaceLevels: Map[ChunkXZ, BlockY] =
      chunkPositions.mappedTo(_.borderElements.map(generator.maxSurfaceAt).min)
