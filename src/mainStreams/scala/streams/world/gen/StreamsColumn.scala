...
final case class StreamsColumn(segment: Segment, pos: BlockXZ, maxFloorLevel: BlockY, minClearLevel: BlockY,
  ...
  private lazy val minFloorY  = segment.downstreamSurfaceLevel :- segment.maxDepth

  def preCarve(chunk: ProtoChunk): Unit =
    given ProtoChunk = chunk
    given bounds: BlockXYZBox = chunk.xyzBounds
    for p <- pos.withNeighbors8.map(_.at(segment.surfaceLevel :+ 1)) do
      if p.isIn(bounds) && !chunk.getOrCreateCarvingMask
          .get(p.x.toInt, p.y.toInt, p.z.toInt) then
        for pc <- p.at(minFloorY).upwardsTo(p.y) do
          preventCarvingAt(pc)
    if !isWall then
  ...
  private def preventCarvingAt(p: BlockXYZ)(using chunk: ProtoChunk): Unit =
    chunk.getOrCreateCarvingMask.set(p.x.toInt, p.y.toInt, p.z.toInt)
  ...
  private def actualFloorLevel(using BlockGetter, BlockYBox): BlockY =
    floorLevelWithNoise(pos.at(maxFloorLevel).downwards.find(_.isLegacySolid).get.y)
  ...
  private def replaceSoil(floorLevel: BlockY)(using WorldGenLevel, BlockYBox): Unit =
    ...
    lazy val rockState = rockStack.headOption.filter(_.isLegacySolid)
